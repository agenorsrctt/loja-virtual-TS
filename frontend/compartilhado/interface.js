...
export function etiqueta(status) {
    ...
    return `<span class="etiqueta ${['ativo', 'inativo', 'pendente', 'pago', 'cancelado'].includes(status) ? status : ''}">${esc({ ativo: 'Ativo', inativo: 'Inativo', pendente: 'Pendente', pago: 'Pago', cancelado: 'Cancelado' }[status] || status)}</span>`;
