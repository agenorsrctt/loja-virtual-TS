...
async function carregar() {
    ...
    const metricas = [['Vendas hoje', dinheiro(soma(hojeVendas)), `${hojeVendas.length} venda(s) concluída(s)`, 'vendas', 'verde'], ['Clientes ativos', clientes.filter(c => c.status === 'ativo').length, 'Conexões que geram negócios', 'clientes', 'azul'], ['Produtos ativos', produtos.filter(p => p.status === 'ativo').length, `${baixo.length} com estoque baixo`, 'produtos', 'roxo'], ['Vendas neste mês', dinheiro(soma(mes)), `${mes.length} venda(s) no período`, 'dinheiro', 'laranja']];
