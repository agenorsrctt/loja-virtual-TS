...
async function carregar() {
    ...
    const validas = vendas.filter(v => v.status === 'pago');

    const hoje = new Date();

    const mesmoDia = (a, b) => a.toDateString() === b.toDateString();

    const soma = lista => lista.reduce((total, v) => total + Math.round(v.valor_total * 100), 0) / 100;
    ...
    const metricas = [['Recebido hoje', dinheiro(soma(hojeVendas)), `${hojeVendas.length} venda(s) paga(s)`, 'vendas', 'verde'], ['Clientes ativos', clientes.filter(c => c.status === 'ativo').length, 'Conexões que geram negócios', 'clientes', 'azul'], ['Produtos ativos', produtos.filter(p => p.status === 'ativo').length, `${baixo.length} com estoque baixo`, 'produtos', 'roxo'], ['Recebido neste mês', dinheiro(soma(mes)), `${mes.length} venda(s) no período`, 'dinheiro', 'laranja']];
