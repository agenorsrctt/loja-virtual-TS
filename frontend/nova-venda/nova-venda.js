...
import { iniciar, esc, dinheiro, icone, estado, erroTela } from '../compartilhado/interface.js';
...
async function finalizar() {
    ...
    try {
        ...
        const resposta = await api('/vendas' + (id ? '/' + id : ''), { method: id ? 'PATCH' : 'POST', body: { cliente_id: cliente.id, itens: [...itens].map(([produto_id, quantidade]) => ({ produto_id, quantidade })) } });
...
async function carregar() {
    ...
    if (id) {
        ...
        original.itens.forEach(i => { if (produto(i.produto_id)) itens.set(i.produto_id, i.quantidade); });
